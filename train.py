...
cfg = dict(
    ...
    n_epochs=30,
    data_root="/root/storage/dataset/samples",
    ...
    outdir="/root/storage/output/gnn_samples",
)
...
def test(epoch, loader):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            if "node" in out:
                predictions_n += list(
                    (results["node"] == batch.y).float().data.cpu().numpy()
                )
                predictions_graph_n += results["graph_node"]
            if "edge" in out:
                predictions_e += list(
                    (results["edge"] == batch.ye).float().data.cpu().numpy()
                )
    ...
    if predictions_n:
        ...
        result["weighted_node"] = scatter(
            torch.from_numpy(np.array(predictions_graph_n).astype("float32")),
            torch.tensor(predictions_graph_weights),
            reduce="mean",
        )
    if predictions_e:
        result["micro_edge"] = np.mean(predictions_e)
        result["macro_edge"] = np.mean(predictions_graph_e)
        result["weighted_edge"] = scatter(
            torch.from_numpy(np.array(predictions_graph_e).astype("float32")),
            torch.tensor(predictions_graph_weights),
            reduce="mean",
        )

    test_acc_graph = np.mean(
        np.array(predictions_graph_e) * np.array(predictions_graph_n)
    )
    logger.info(
        "Test %4d: loss: %.4f test acc: %.4f path acc: %.4f"
        % (epoch, test_loss, test_acc, test_acc_graph)
    )
...
def train(epoch):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if epoch % log_epoch_interval == 0 and batch_idx % log_interval == 0:
            ...
            print(
                "Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.4f}\t Acc: {:.4f}".format(
                    epoch,
                    batch_idx * len(batch),
...
train_loader = DataLoader(
    GraphDataset(cfg["data_root"] + "/train"),
...
valid_loader = DataLoader(
    GraphDataset(cfg["data_root"] + "/val"),
    batch_size=batch_size_test,
    follow_batch=["edge_index"],
    num_workers=cfg["num_workers"],
)
test_loader = DataLoader(
    GraphDataset(cfg["data_root"] + "/test"),
    batch_size=batch_size_test,
    follow_batch=["edge_index"],
    num_workers=cfg["num_workers"],
)
